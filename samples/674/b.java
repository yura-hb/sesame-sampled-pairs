...
class WindowsPreferences extends AbstractPreferences {
	...
	if (nativeHandle == NULL_NATIVE_HANDLE) {
	    throw new BackingStoreException(
		    "Could not open windows registry node " + byteArrayToString(windowsAbsolutePath()) + " at root 0x"
    ...
    */
	...
	if (result != ERROR_SUCCESS) {
	    logger().warning("Could not close windows registry node " + byteArrayToString(windowsAbsolutePath())
		    + " at root 0x" + Long.toHexString(rootNativeHandle())
	...
	if (windowsAbsolutePath.length &lt;= MAX_WINDOWS_PATH_LENGTH + 1) {
	    long[] result = WindowsRegOpenKey1(rootNativeHandle(), windowsAbsolutePath, mask1);
		...
		logger().warning("Could not open windows registry node " + byteArrayToString(windowsAbsolutePath())
			+ " at root 0x" + Long.toHexString(rootNativeHandle())
		...
		if (result[ERROR_CODE] == ERROR_ACCESS_DENIED) {
		    throw new SecurityException(
			    "Could not open windows registry node " + byteArrayToString(windowsAbsolutePath())
				    + " at root 0x" + Long.toHexString(rootNativeHandle()) + ": Access denied");
		}
	    }
	...
	if (result[ERROR_CODE] == ERROR_SUCCESS) {
	    ...
	    logger().warning("Trying to recreate Windows registry node " + byteArrayToString(subKey) + " at root 0x"
	...
	if (windowsRelativePath.length &lt;= MAX_WINDOWS_PATH_LENGTH + 1) {
	    long[] result = WindowsRegOpenKey1(nativeHandle, windowsRelativePath, mask1);
	    if (result[ERROR_CODE] == ERROR_ACCESS_DENIED && mask2 != mask1)
	    ...
	    long nextNativeHandle = openKey(nativeHandle, nextRelativeRoot, mask1, mask2);
	    if (nextNativeHandle == NULL_NATIVE_HANDLE) {
		return NULL_NATIVE_HANDLE;
	    }
	    long result = openKey(nextNativeHandle, nextRelativePath, mask1, mask2);
     ...
     */
    private static native long[] WindowsRegOpenKey(long hKey, byte[] subKey, int securityMask);
